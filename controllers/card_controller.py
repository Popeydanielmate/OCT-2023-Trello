...
from init import db
from model.card import Card, cards_schema, card_schema
from model.comment import Comment, comment_schema
...
def get_one_card(card_id):
    stmt = db.select(Card).filter_by(id=card_id) # similar to select * from cards where id=4;
    card = db.session.scalar(stmt)
    if card:
        return card_schema.dump(card)
    else:
        return {"error": f"Card with id {card_id}"}, 404
    
#  http://localhost:6060/cards
...
#  "/cards/<int:card_id>/coments/<int:comment_id>" -> PUT, PATCH, DELETE
